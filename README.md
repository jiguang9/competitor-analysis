...
| ④ 竞品周报* | "生成竞品周报"、"更新上次竞品报告" | 本期动作、影响分析、跟/缓/不跟判断 |

\* "周报"是名字，不是承诺——见下一节，实际间隔不假设是 7 天。

## 关于"竞品周报"

竞品周报由用户**主动运行**生成，不是后台监控或自动通知。v1.0 不提供定时调度、抓取脚本或推送能力——"周报"描述的是报告的周期性用途，不是自动化程度，也不代表两次运行之间正好相隔一周。首次运行会建立基线；后续运行会与最近一次有效基线比较，全部来源失败时明确输出"本期数据不足"而不是"本周无变化"。

因为没有固定排期，每份报告都会明确写出**本期核验日期、上次基线日期、两者实际相隔天数**，正文一律用"本期/上期"而不是"本周/上周"——除非你自己确认两次运行确实正好相隔一周。间隔明显偏离一周（比如超过一个月，或不到两天）时，报告会提醒这会影响"变化"的解读方式。
